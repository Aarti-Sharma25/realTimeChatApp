...
**4️⃣ Set up environment variables** — create a `.env` file in `backend/` with:
```
PORT=8000
...
**5️⃣ Set up the MongoDB Atlas Vector Search index** on the `messages` collection (index name: `message_vector_index`):
```json
{
  "fields": [
    {
